...
func (h *ActivityRepo) GetActivitesByYear(Userid string, Year string) ([]models.Activity, error) {
	...
	log.Default().Output(1, toDate+" "+fromDate)
	rows, err := h.db.Query("SELECT activityid, userid, name, category, date_of_upload, certificate from activities WHERE userid = $1 AND date_of_upload >= $2 AND date_of_upload <= $3", Userid, fromDate, toDate)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (h *ActivityRepo) Save(userid string, name string, category string, certificate string) error {
	_, err := h.db.Exec("INSERT INTO activities(userid, name, category, certificate) VALUES($1, $2, $3, $4)", userid, name, category, certificate)
	return err
}
