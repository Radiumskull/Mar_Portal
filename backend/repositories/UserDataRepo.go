...
func (r *UserDataRepo) FindByID(ID string) (*models.UserData, error) {
	...
	err := r.db.QueryRow("select userid, name, yoa, department, points from userdata where userid = $1", ID).Scan(&user.Userid, &user.Name, &user.YOP, &user.Dept, &user.Points)
	return &user, err
}
...
func (r *UserDataRepo) Save(user *models.UserData) error {
	err := r.db.QueryRow("insert into userdata(userid, name, yoa, department) values($1, $2, $3, $4)", user.Userid, user.Name, user.YOP, user.Dept).Err()
	return err
}
